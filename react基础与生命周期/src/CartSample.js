...
// 函数型子组件
// props.addCount表示调用父组件的addCount方法
function Cart(props) {
  return (    // 多行需要用小括号包裹
    <table>
      <tbody>
        {props.data.map(d => (
          <tr key={d.text}>
            ...
            <td>
              <button>-</button>
              {d.count}
              {/* 函数传参需要使用下面的箭头函数写法，但是无参数的时候就可以直接调用 */}
...
export default class CartSample extends Component {
  ...
  // setState用于改变state中数据值，如果不依赖于改变之前的值就直接传参使用，否则需要传入prevState 箭头函数方式调用
  addToCart(good) {
    // 为了保证视图更新，数组要保证引用地址变化，因此每次深复制一个新数组（类似于vue无法watch数组一样）
